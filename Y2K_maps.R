...
# Year 2000 maps -------------------------------
{
  ...
  gp_surv = lapply(which(daymet_years %in% surv_layers), function(x) gplot(subset(survival_brick, x), maxpixels = maxpixels))
  names(gp_surv) = surv_layers
  ...
  get_pine_gg = function(kill_year, sz = 0.01, alph = 0.5)
  {
    ...
    dtt[, kill_log := log(kill1)]; #dtt[, summary(kill_log)]
    return (  geom_point(data = dtt, aes(x, y, color = kill_log), size = sz, alpha = alph, show.legend = F))
  }
  ...
  scg_1 = scale_color_gradientn(
    name = "Pines killed per 1 ha",
      colours = cols_pine_2, na.value = rgb(0, 0, 0, 0),
      breaks = log(breaks_pine_2k), labels = breaks_pine_2k)
  
  kill_years
  # Grobs of kill plots ----
  kill_grobs = lapply(kill_years, function(x)  ggplot() + t1 + coord_equal(xlim = c(-2e6, 0.2e6), ylim = c(-1.2e6, 1e6)) + 
